...
class CelebADataset(ConfounderDataset):
    ...
    def __init__(
        ...
        df1 = pd.read_csv(os.path.join(self.root_dir, "list_eval_partition.csv"))
        df2 = pd.read_csv(os.path.join(self.root_dir, "list_attr_celeba.csv"))
        merged_df = pd.merge(df1, df2)
        merged_df["split"] = merged_df["partition"]
        merged_df.to_csv(os.path.join(self.root_dir, metadata_csv_name), header=True, index=False)
