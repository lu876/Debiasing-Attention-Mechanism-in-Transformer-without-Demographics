...
def _create_pipe(estimator, dtypes, sparse=False, with_mean=True):
    # Convert boolean mask to list of column indices for categorical columns
    categorical_indices = [i for i, is_cat in enumerate(is_categorical(dtypes)) if is_cat]
    
    # List of column indices for non-categorical columns
    non_categorical_indices = [i for i, is_cat in enumerate(is_categorical(dtypes)) if not is_cat]

    # Define transformers
    transformers = [
        ("categorical", OneHotEncoder(sparse=sparse), categorical_indices),
        ("scaler", StandardScaler(with_mean=with_mean), non_categorical_indices)
    ]
